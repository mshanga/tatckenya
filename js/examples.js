...
function disableScroll(elem) {
  ...
  pos = { x: $(elem).scrollLeft(), y: $(elem).scrollTop() };
...
$(document).ready(function() {
  var $menu = $("ul#menu").wrap("<div>"),
    $menuwrap = $menu.parent().addClass("menuwrap");
  if ($menu.length > 0) {
    ...
    $.each(MENU, function(mainID, submenu) {
      var path = isRoot ? mainID : "../" + mainID;
      ($li = $("<li>").appendTo($menu)),
        ($a = $("<a href='" + path + "'>" + submenu.title + "</a>").appendTo(
          $li
        )),
      ...
      $.each(submenu.sub, function(subURL, subTitle) {
        var $li = $("<li>").appendTo($ul_sub),
          $a = $(
            "<a href='" + path + "/" + subURL + "'>" + subTitle + "</a>"
          ).appendTo($li);
    ...
    if (Modernizr.touch) {
      ...
      $("section#titlechart").height($(window).height());
    }
  }
  $("a.viewsource").each(function() {
    var demoSelector = ".demowrap, section.demo:not(.demowrap .demo)",
      $demoElements = $(demoSelector),
      $relevantCode =
        $demoElements.length <= 1
          ? $demoElements
          : $(this)
              .parents(demoSelector + ", div#example-wrapper, body")
              .first();
  ...
  $.each(badges, function(key, badge) {
    $("h1.badge." + key).attr(
      "title",
      "This example requires the " + badge.name + "."
    );
  });
});
...
$(document).on("mousedown", ".slider, .move", function(e) {
  if (e.which === 1) {
    var $this = $(this);
    if ($this.is(".slider") || e.target == this) {
      e.stopPropagation();
...
$(document).on("mouseup mousemove", function(e) {
  $(".move.dragging").each(function(f) {
    ...
    if (data) {
      $(this).css({ top: data.top + e.pageY, left: data.left + e.pageX });
    }
  });
  $(".slider.dragging").each(function(f) {
    var data = $(this).data("drag");
    if (data) {
      var pos = e.pageX - data.left,
        width = $(this).width(),
...
$(document).on("orientationchange", function(e) {
  if ($("#example-wrapper.horizontal").length > 0) {
    $("meta[name='viewport']").attr(
      "content",
      (window.orientation === 0 ? "width" : "height") + "=500"
    );
  }
});
